...
function displayconvo(userOneFriendData, convoId, currentLogedInUserId) {
  ...
  onechat.setAttribute(
    "data-profilepic-id",
    userOneFriendData["profile_pic_id"]
  );
    ...
    .then(function (response) {
      if (response.status === 203) {
        return response.json().then(function (data) {
          ...
          let convocontainers = displayconvoonload(
            allUserFriendsData,
            currentLoggedUserConvoIds,
            currentLogedInUserId
          );
...
function displayconvoonload(
  ...
  for (let i = 0; i < allUserFriendsData.length; i++) {
    ...
    for (let j = 0; j < currentLoggedUserConvoIds.length; j++) {
      ...
      if (
        doesConvorIdMatchFriendsIdFromDb(
          oneConvoId["convor_id"],
          oneUserFriendData["user_id"],
          currentLogedInUserId
        )
      ) {
        const onechat = document.createElement("div");
        ...
        onechat.setAttribute(
          "data-profilepic-id",
          oneUserFriendData["profile_pic_id"]
        );
        onechat.setAttribute(
          "data-receiver-username",
          oneUserFriendData["username"]
        );
        onechat.setAttribute("data-currentLogedInUserId", currentLogedInUserId);
...
function doesConvorIdMatchFriendsIdFromDb(convorId, friendsId, loggedUserId) {
  const trimmedConvorId = convorId.replace(
    new RegExp(`^${loggedUserId}|${loggedUserId}$`, "g"),
    ""
  );
